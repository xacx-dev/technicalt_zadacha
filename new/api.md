...
# 4. Блок: Логика синхронизации данных и кэширования

## 4.1. Общие принципы синхронизации
Ночная синхронизация проводится ежедневно в выбранный интервал (например, 02:00–04:00).  
Цели синхронизации:
- уменьшить нагрузку на API 1С,
- ускорить работу бота,
- обеспечить актуальность данных,
- позволить боту работать даже при временной недоступности 1С.

## 4.2. Какие данные синхронизируются
- Пользователи (ФИО, телефон, статус)
- Баллы (баланс и блокировки)
- История операций по баллам
- Магазины
- Персональные предложения и акции
- Реферальные данные

## 4.3. Логика ночной синхронизации
1. Получение полного списка пользователей:
   - GET /api/customers/all  
2. Обновление данных в локальной БД.
4. Получение новых операций по баллам:
   - GET /api/customer/{phone}/history
5. Получение магазинов:
   - GET /api/stores
6. Получение персональных предложений:
   - GET /api/offers
7. Получение реферальной информации:
   - GET /api/customer/{phone}/referral

## 4.4. Механизм обратной связи с 1С
При невозможности обновить часть данных — система делает повтор с использованием backoff,  
ошибки фиксируются в журнал синхронизации.

## 4.5. Ограничения нагрузки
- Ограничение: не более 10 RPS.
- Повтор запросов при ошибках 429/timeout.
---

# 5. Локальное кэширование

## 5.1. Данные, которые берутся из локальной БД
| Данные | Актуальность | Когда обращаться к API |
|--------|--------------|-------------------------|
| Магазины | 24 часа | Только при кнопке "Обновить" или при ночной синхронизации |
| Акции/предложения | 24 часа | Только nightly sync |
| Реферальные данные | 24 часа | Только при изменениях через webhook |
| История | Постоянно | Последние 24 часа всегда запрашиваются у 1С |
| Баланс | 15 минут | Если запрос пользователя или свежий webhook |

## 5.2. Логика получения баланса
1. Проверяется время последнего обновления.
2. Если прошло менее 15 минут — используется БД.
3. Если прошло более 15 минут — выполняется запрос к API.
4. После получения — обновление БД.

## 5.3. История операций
- Вся история старше 24 часов — из локальной БД.
- Последние 24 часа — обязательный запрос к API.
